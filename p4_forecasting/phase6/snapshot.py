"""Phase-6 source immutability snapshotting (read-only for consumed sources).

Hashes every non-Phase-6 file under the project (excluding the huge
PS70-main.zip archive, bytecode and pytest caches) so we can prove later that
P1 / Phase-1 / Phase-2 / Phase-3 / Phase-4 / Phase-5 and everything outside
Phase-6 were left unchanged.

Buckets (paths relative to cyclone-project/):
    p1               p4_forecasting/_source_p1, canonical, canonical_chrono
    phase1           p4_forecasting/canonical, canonical_chrono, audit,
                     reports, scripts, logs
    phase2           p4_forecasting/phase2
    phase3           p4_forecasting/phase3
    phase4           p4_forecasting/phase4
    phase5           p4_forecasting/phase5
    outside_phase6   every key (i.e. the whole tree outside phase6)
"""
...
def master_snapshot() -> dict:
    """Full non-Phase6 tree snapshot keyed relative to cyclone-project/."""
    return snapshot_tree(
        PROJECT, PROJECT,
        skip_prefixes=("p4_forecasting/phase6/", "p4_forecasting/phase6"),
    )
...
def write_before(path: Path = RESULTS / "source_hashes_before.json") -> dict:
    """Snapshot every consumed source, taken before any Phase-6 step that
    could conceivably touch project data."""
    master = master_snapshot()
    payload = {
        "schema": "phase6 v1",
        "recorded_at": datetime.now(timezone.utc).isoformat(),
        "policy": ("SHA256 of every non-Phase-6 file under the project "
                   "(excluding PS70-main.zip, __pycache__, *.pyc, "
                   ".pytest_cache and p4_forecasting/phase6 itself)."),
        "project_root": str(PROJECT),
        "excluded": ["PS70-main.zip", "__pycache__", "*.pyc",
                     ".pytest_cache", "p4_forecasting/phase6"],
...
def bucket_changes(before_master: dict, after_master: dict) -> dict:
    """Per-bucket changed-file lists plus the outside_phase6 total."""
    out = {"outside_phase6": changed_files(before_master, after_master)}
...
def write_after_report(after_master: dict, status: dict,
    ...
    report = {
        ...
        "phase5_changed": len(status["changes"]["phase5"]),
        "outside_phase6_changed": len(status["changes"]["outside_phase6"]),
        "changed_files": status["changes"],
        "n_files_after": len(after_master),
        "note": note or ("No P1 / Phase-1/2/3/4/5 or outside-Phase-6 file "
                         "changed."),
    }
    path.parent.mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="Phase-6 source snapshot")
    ap.add_argument("--before", action="store_true",
                    help="write source_hashes_before.json (run before impl)")
    ap.add_argument("--check", action="store_true",
                    help="report whether the current tree matches BEFORE")
    args = ap.parse_args()
    if args.before:
        p = write_before()
        print(f"[phase6-snapshot] BEFORE recorded: {p['n_files']} files -> "
